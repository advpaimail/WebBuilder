...
const genericCreateAccount = async (
  ...
  if (dbUser.error.code !== "PGRST116") {
    const errMsg = [dbUser.error.message, dbUser.error.details, dbUser.error.hint].filter(Boolean).join("; ");
    console.error("PostgREST/User error:", dbUser.error);
    throw new Error(errMsg ? `User not found: ${errMsg}` : "User not found");
  }
